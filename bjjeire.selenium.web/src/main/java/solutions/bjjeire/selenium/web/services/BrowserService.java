...
public class BrowserService extends WebService {
  ...
  public BrowserService(
      DriverService driverService, JavaScriptService javaScriptService, WebSettings webSettings) {
    super(driverService);
  ...
  public void waitUntilPageLoadsCompletely() {
    ...
    log.debug(
        "Waiting for page load to be complete",
        StructuredArguments.keyValue("timeoutSeconds", pageLoadTimeout));
    try {
      WebDriverWait wait =
          new WebDriverWait(getWrappedDriver(), Duration.ofSeconds(pageLoadTimeout));
      wait.until(
          driver ->
      ...
      log.warn(
          "Timed out waiting for page to load completely",
          StructuredArguments.keyValue("timeoutSeconds", pageLoadTimeout),
          ex);
    }
  }
  ...
  public void assertLandedOnPage(String partialUrl) {
    ...
    Assert.assertTrue(
        currentBrowserUrl.contains(partialUrl.toLowerCase()),
        String.format(
            "The expected partialUrl: '%s' was not found in the PageUrl: '%s'",
            partialUrl, currentBrowserUrl));
  }
  ...
  public void assertUrl(String fullUrl) {
    ...
    try {
      ...
      Assert.assertEquals(
          actualUri.toASCIIString(),
          expectedUri.toASCIIString(),
          "Expected URL is different than the Actual one.");
    } catch (URISyntaxException e) {

      log.error(
          "Invalid URL syntax for comparison",
          StructuredArguments.keyValue("actualUrl", currentBrowserUrl),
          StructuredArguments.keyValue("expectedUrl", fullUrl),
          e);
