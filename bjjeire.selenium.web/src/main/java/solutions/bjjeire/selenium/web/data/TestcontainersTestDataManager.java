 ...
 * This implementation aligns with the TestDataManager interface for use in parallel execution environments.
 * The core strategy for isolation is to wipe the database clean before each scenario runs.
 */
@Service
@Profile("staging")
public class TestcontainersTestDataManager implements TestDataManager {
    ...
    private final ObjectMapper objectMapper; // To deserialize the response
    private final String testApiBaseUrl;
    private MongoDBContainer mongoDBContainer;

    public TestcontainersTestDataManager(RestTemplate restTemplate, ObjectMapper objectMapper, @Value("${web-settings.backendApiUrl}") String baseUrl) {
    ...
    public void startContainer() {
        ...
        // This sets the connection string for the backend application to use this container.
     ...
     * `authenticate` contract by returning a dummy token, which is not needed for this implementation.
     *
    ...
    public List<String> seedEvents(List<BjjEvent> events, String authToken) {
        ...
        // This assumes the backend's test endpoint /seed/events accepts a list of events
        // and returns a list of the created BjjEvent objects (or at least their IDs).
        ResponseEntity<List<BjjEvent>> response = restTemplate.postForEntity(url, events, (Class<List<BjjEvent>>)(Class<?>)List.class);
     ...
     * completely at the beginning of the *next* scenario. This is a robust pattern
     * that ensures a clean state even if a test fails before teardown.
     *
    ...
    public void teardown(List<String> createdEventIds, String authToken) {
        log.info("Testcontainers teardown is a no-op. Cleanup is handled before the next scenario runs.");
    }
}