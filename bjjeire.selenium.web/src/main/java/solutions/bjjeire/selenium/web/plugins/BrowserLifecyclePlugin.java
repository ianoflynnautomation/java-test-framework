...
public class BrowserLifecyclePlugin implements Plugin {
  ...
  private final ThreadLocal<BrowserConfiguration> previousBrowserConfiguration =
      new ThreadLocal<>();
  private final ThreadLocal<Boolean> isBrowserStartedCorrectly =
  ...
  public void preBeforeTest(TestResult testResult, Method memberInfo) {
    log.debug(
        "Executing pre-before-test hook",
  ...
  public void postAfterTest(
    ...
    log.debug(
        "Executing post-after-test hook",
        StructuredArguments.keyValue("methodName", memberInfo.getName()),
        StructuredArguments.keyValue("testResult", testResult.name()));
  ...
  public void preBeforeScenario(ScenarioContext context) {
    ...
    log.debug(
        "Executing pre-before-scenario hook",
  ...
  public void postAfterScenario(ScenarioContext context) {
    log.debug(
        "Executing post-after-scenario hook",
        StructuredArguments.keyValue("scenarioName", context.getScenarioName()),
    ...
    if (shouldRestartBrowser()) {
      ...
      try {
        ...
        throw new RuntimeException(
            "Failed to initialize WebDriver. Check logs for the original exception.", ex);
      }
  ...
  private void handleBrowserShutdown(TestResult testResult, BrowserConfiguration config) {
    ...
    switch (config.getLifecycle()) {
      case REUSE_IF_STARTED:
        log.debug(
            "Browser lifecycle is REUSE_IF_STARTED",
      ...
      case RESTART_ON_FAIL:
        if (testResult != TestResult.FAILURE) {
          ...
          log.debug(
              "Browser lifecycle is RESTART_ON_FAIL and test passed",
  ...
  private BrowserConfiguration getBrowserConfiguration(Method memberInfo) {
    BrowserConfiguration classConfig =
        getExecutionBrowserClassLevel(memberInfo.getDeclaringClass());
    ...
    String testFullName =
        String.format("%s.%s", memberInfo.getDeclaringClass().getName(), memberInfo.getName());
  ...
  private BrowserConfiguration getExecutionBrowserMethodLevel(Method memberInfo) {
    ...
    return new BrowserConfiguration(
        annotation.browser(), annotation.deviceName(), annotation.lifecycle());
  }
