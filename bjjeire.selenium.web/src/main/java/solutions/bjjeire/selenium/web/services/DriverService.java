...
public class DriverService {
  ...
  private static final List<WebDriver> ALL_DRIVERS =
                      ...
                      .forEach(
                          driver -> {
                            try {
                              if (driver != null) {
                                log.debug(
                                    "Shutting down driver instance",
                                    StructuredArguments.keyValue("driverInstance", driver));
                                driver.quit();
                              }
                            } catch (Exception e) {
                              log.error(
                                  "Error while shutting down a WebDriver instance during JVM shutdown.",
                                  e);
                            }
                          });
  ...
  public WebDriver start(BrowserConfiguration configuration) throws Exception {
    ...
    if (webDriverThreadLocal.get() != null) {
      log.warn(
          "A WebDriver instance already exists for this thread. Closing it before starting a new one.");
      close();
    }
  ...
  private WebDriver createDriver(BrowserConfiguration configuration) throws Exception {
    String executionType =
        Optional.ofNullable(webSettings.getExecutionType())
            .orElseThrow(
                () ->
                    new Exception("The 'executionType' property is not set in the configuration."))
            .toLowerCase();
  ...
  private void configureDriver(WebDriver driver, BrowserConfiguration configuration) {
    ...
    if (configuration.getHeight() != 0 && configuration.getWidth() != 0) {
      driver
          .manage()
          .window()
          .setSize(new Dimension(configuration.getWidth(), configuration.getHeight()));
    } else {
      driver.manage().window().maximize();
    }
    log.info(
        "Window resized",
        StructuredArguments.keyValue("windowSize", driver.manage().window().getSize()));
  }
  ...
  private WebDriver initializeLocalDriver(BrowserConfiguration config) throws Exception {
    log.debug(
        "Initializing driver in 'local' mode",
    ...
    return switch (config.getBrowser()) {
      ...
      default ->
          throw new Exception("Unsupported browser for local execution: " + config.getBrowser());
    };
  }

  private WebDriver initializeGridDriver(BrowserConfiguration config, String providerName)
      throws Exception {
    ...
    try {
      ...
      log.info(
          "Initializing driver in grid mode",
  ...
  private WebDriver initializeCloudDriver(BrowserConfiguration config, String providerName)
      throws Exception {
    GridSettings gridSettings = findGridSettings(providerName);
    MutableCapabilities capabilities = getBrowserOptions(config.getBrowser());
    ...
    try {
      String gridUrl = getUrl(gridSettings.getUrl());
      log.info(
          "Initializing driver in cloud grid mode",
  ...
  private void addGridOptions(MutableCapabilities capabilities, GridSettings gridSettings) {
    gridSettings.getArguments().stream()
        ...
        .forEach(
            entry -> capabilities.setCapability(entry.getKey(), resolveValue(entry.getValue())));
  }
