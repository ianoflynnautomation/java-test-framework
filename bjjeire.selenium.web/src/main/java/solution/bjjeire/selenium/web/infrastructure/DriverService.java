...
import solution.bjjeire.selenium.web.configuration.GridSettings;
...
import solutions.bjjeire.core.utilities.ShutdownManager;
...
public class DriverService {
    private static ThreadLocal<DriverManager> DRIVER_MANAGER = new ThreadLocal<DriverManager>();
    ...
    public static WebDriver getWrappedDriver() {
        return DRIVER_MANAGER.get().wrapppedDriver;
    }

    public static void setWrappedDriver(WebDriver driver) {
        DRIVER_MANAGER.get().wrapppedDriver = driver;
    }
    ...
    private static class DriverManager {
        ...
        private WebDriver wrapppedDriver;
        private boolean isBuildNameSet;
...
//                Log.info(String.format("Window resized to dimensions: %s", driver.manage().window().getSize().toString()));
                wrapppedDriver = driver;

                return driver;
            } else
                return wrapppedDriver;
        }
...
//                    deviceMetrics.put("width", browserConfiguration.getDeviceName().getWidth());
//                    deviceMetrics.put("height", browserConfiguration.getDeviceName().getHeight());
//                    deviceMetrics.put("pixelRatio", browserConfiguration.getDeviceName().getScaleFactor());
//
