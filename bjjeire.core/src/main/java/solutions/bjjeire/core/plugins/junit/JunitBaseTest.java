...
public abstract class JunitBaseTest extends UsesPlugins {
  ...
  public void beforeClassCore() {
    ...
    log.info(
        "Executing @BeforeAll phase", StructuredArguments.keyValue("testClass", testClassName));
    try {
  ...
  public void afterMethodCore(TestInfo testInfo) {
    ...
    try {
      ...
      Throwable testException =
          CURRENT_TEST_RESULT.get() == TestResult.FAILURE
              ? new RuntimeException("Test Failed")
              : null;
      PluginExecutionEngine.postAfterTest(
          CURRENT_TEST_RESULT.get(), CURRENT_TEST_TIME_RECORD.get(), methodInfo, testException);
      afterEach();
    } catch (Exception e) {
      log.error(
          "Error during @AfterEach phase",
          StructuredArguments.keyValue("testName", testInfo.getDisplayName()),
          e);
    }
  }
