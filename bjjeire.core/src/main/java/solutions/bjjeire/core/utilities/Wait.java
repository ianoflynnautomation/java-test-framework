...
public class Wait {
    public static void retry(Runnable action, int timesToRetry, long sleepInterval,
            Class<? extends Throwable>... exceptionsToIgnore) {
        Wait.retry(action, timesToRetry, sleepInterval, true, exceptionsToIgnore);
    }

    public static void retry(Runnable action, int timesToRetry, long sleepInterval, boolean shouldThrowException,
            Class<? extends Throwable>... exceptionsToIgnore) {
        int repeat = 0;
        while (repeat <= timesToRetry) {
            repeat++;
            if (retry(action, Duration.ofSeconds(sleepInterval), Duration.ofSeconds(sleepInterval),
                    shouldThrowException, exceptionsToIgnore)) {
                break;
            }
        }
    }
    ...
    public static boolean retry(Runnable action, Duration timeout, Duration sleepInterval, Boolean shouldThrowException,
            Class<? extends Throwable>... exceptionsToIgnore) {
    ...
    public static void retry(Runnable action, Duration timeout, Duration sleepInterval,
            Class<? extends Throwable>... exceptionsToIgnore) {
    ...
    public static boolean forConditionUntilTimeout(Comparator condition, long timeoutInMilliseconds,
