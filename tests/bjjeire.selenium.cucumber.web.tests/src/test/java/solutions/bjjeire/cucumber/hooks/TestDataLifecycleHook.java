...
public class TestDataLifecycleHook {
  ...
  public void afterScenario() {
    log.info(
        "Executing scenario teardown hook",
        StructuredArguments.keyValue("eventName", "teardownHookStart"));
    String authToken = scenarioContext.getAuthToken();
    if (authToken == null || authToken.isBlank()) {
      log.debug(
          "Skipping data teardown due to missing auth token",
    ...
    if (!eventIds.isEmpty()) {
      log.info(
          "Tearing down BJJ event entities",
    ...
    log.info(
        "Scenario teardown hook finished",
        StructuredArguments.keyValue("eventName", "teardownHookEnd"));
  }
}
