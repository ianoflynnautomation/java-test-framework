...
public class EventTests extends JunitWebTest {
        ...
        testDataManager.seed(
            List.of(
                ...
                BjjEventFactory.createBjjEvent(
                    b -> b.county(County.Dublin).name("Dublin Open Mat"))),
            authToken));

    eventsPage.open();
    eventsPage
        .selectCounty(countyStr)
  ...
  public void filterByEventType_shouldShowOnlyEventsOfSelectedType() {
    ...
    eventsPage.open();
    eventsPage
        .selectFilter(eventTypeToFilter)
  ...
  @CsvSource({"Cork, Seminar", "Cork, Tournament"})
  public void filterByCountyAndType_shouldShowMatchingEvents(
      String countyStr, String eventTypeStr) {
    ...
    createdEventIds.addAll(
        testDataManager.seed(
            List.of(
                BjjEventFactory.createBjjEvent(
                    b -> b.county(county).type(BjjEventType.SEMINAR).name("Cork Seminar")),
                BjjEventFactory.createBjjEvent(
                    b -> b.county(county).type(BjjEventType.TOURNAMENT).name("Cork Open Mat")),
                BjjEventFactory.createBjjEvent(
                    b ->
                        b.county(County.Dublin).type(BjjEventType.SEMINAR).name("Dublin Seminar"))),
            authToken));
  ...
  public void filterForCountyWithNoMatchingEvents_shouldShowEmptyList() {
    ...
    createdEventIds.addAll(
        testDataManager.seed(
            List.of(BjjEventFactory.createBjjEvent(b -> b.county(County.Dublin))), authToken));

    eventsPage.open();
    eventsPage.selectCounty("Clare").assertNoDataInList();
  }
  ...
  public void filterWhenNoEventsExist_shouldShowEmptyList() {
    ...
    eventsPage.open();
    eventsPage.selectCounty("Wexford").assertNoDataInList();
  }
}
